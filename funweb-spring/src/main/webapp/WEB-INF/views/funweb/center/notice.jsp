...
<body>
	<%--
List<Board> l = (List<Board>) request.getAttribute("list");
Map<String, Integer> map = (Map<String, Integer>) request.getAttribute("pageInfoMap");
SimpleDateFormat ymd = new SimpleDateFormat("yyyy.MM.dd");
SimpleDateFormat hms = new SimpleDateFormat("HH:mm:ss");
int count = 0;
System.out.println("게시글 총 " + l.size() + "개");
System.out.println("Page : " + map.toString());
--%>
	<div id="wrap">
		<!-- 헤더들어가는 곳 -->
		...
		<article>
			<c:choose>
				<c:when test="${search == null || search.length() == 0 }">
					<h2>Notice - ${pageInfoMap.allRowCount }개</h2>
				</c:when>
				<c:otherwise>
			...
			<table id="notice">
				...
				<c:choose>
					...
					<c:otherwise>
						...
						<tr	onclick="location.href='/board/detail?num=${v.num }&name=${v.name }&pageNum=${pageInfoMap.pageNum}';">
							<td>${v.num }</td>
							<td class="title left">
							<c:if test="${v.reLev > 0 }">
							...
							<a href="/board/detail?num=${v.num }&name=${v.name }&pageNum=${pageInfoMap.pageNum }">
									${v.subject }</a></td>
							<td>${v.name }</td>
			...
			<c:if test="${sessionID != null }">
				<div id="table_search">
					...
					<!-- <input type="button" value="FileUpload" class="btn" onclick="location.href='boardFieWriteForm.do';"> -->
				</div>
			</c:if>
			<div id="table_search">
			...
			<div id="page_control">
				...
				<c:if test="${pageInfoMap.allRowCount > 0 }">
					...
					<c:if test="${pageInfoMap.startPage > pageInfoMap.pageBlockSize }">
						<a href="/board/list?pageNum=1&search=${search }">1</a>...
						<a href="/board/list?pageNum=${pageInfoMap.startPage - 1 }&search=${search }">Prev</a>
					</c:if>

					<c:forEach begin="${pageInfoMap.startPage }"
						end="${pageInfoMap.endPage }" step="1" varStatus="s">
						<a href="/board/list?pageNum=${s.current }&search=${search }">
							<c:choose>
					...
					<c:if test="${pageInfoMap.endPage < pageInfoMap.maxPage }">
						<a href="/board/list?pageNum=${pageInfoMap.endPage + 1 }&search=${search }">Next</a>...
						<a href="/board/list?pageNum=${pageInfoMap.maxPage }&search=${search }">${pageInfoMap.maxPage }</a>
					</c:if>
